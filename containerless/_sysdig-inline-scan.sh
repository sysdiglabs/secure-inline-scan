...
get_and_validate_analyzer_options() {
    ...
    getopt -T || RETCODE=$?
    if [ "$RETCODE" != "4" ]; then
        exit_with_error "GNU getopt is required. Please check installed version of getopt"
    fi
...
check_dependencies() {
    ...
    if [[ "${BASH_VERSINFO:-0}" -lt 4 ]]; then
        exit_with_error "Bash version 4 or higher required"
    elif [[ ! $(which skopeo) ]]; then
        exit_with_error "Skopeo is not installed or cannot be found in \$PATH"
    elif [[ ! $(which jq) ]]; then
        exit_with_error "jq is not installed or cannot be found in \$PATH"
...
EOF
    ...
    MANIFEST=$(skopeo inspect --override-os linux "${SKOPEO_REGISTRY_CONF[@]}" "${SKOPEO_AUTH[@]}" --raw "${SOURCE_IMAGE}" 2> "${TMP_PATH}"/err.log) || find_image_error "${IMAGE_NAME}"
    INSPECT=$(skopeo inspect --override-os linux "${SKOPEO_REGISTRY_CONF[@]}" "${SKOPEO_AUTH[@]}" "${SOURCE_IMAGE}" 2> "${TMP_PATH}"/err.log) || find_image_error "${IMAGE_NAME}"
...
convert_image() {
    if [[ "${SOURCE_IMAGE}" != "oci:${DEST_IMAGE_PATH}" ]]; then
        ...
        skopeo copy --override-os linux "${SKOPEO_REGISTRY_CONF[@]}" "${SKOPEO_COPY_AUTH[@]}" "${SOURCE_IMAGE}" "oci:${DEST_IMAGE_PATH}" 2> "${TMP_PATH}"/err.log | print_info_pipe "  " || find_image_error "${IMAGE_NAME}"
    fi
}
...
perform_analysis() {
    ...
    ANALYZE_CMD+=('anchore-manager --configdir . analyzers exec ${DEST_IMAGE_PATH} ${TMP_PATH}/image-analysis-archive.tgz')
