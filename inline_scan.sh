...
get_and_validate_analyzer_options() {
    ...
    if [[ ! $(which docker) ]]; then
        ...
        display_usage_analyzer >&2
        exit 1
    elif ! curl -s "${CURL_AUTH}" --fail -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_ANCHORE_URL}/images" > /dev/null; then
...
post_analysis() {
    ...
    mkdir -p /tmp/sysdig
    HCODE=$(curl -sS "${CURL_AUTH}" --output /tmp/sysdig/sysdig_output.log --write-out "%{http_code}" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_SCANNING_URL%%/}/account")
...
get_scan_result_code_by_id() {
    GET_CALL_STATUS=$(curl -s "${CURL_AUTH}" -o /dev/null --write-out "%{http_code}" --header "Content-Type: application/json" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_ANCHORE_URL}/images/by_id/${SYSDIG_IMAGE_ID}/check?tag=$FULLTAG&detail=${DETAIL}")
}
...
get_scan_result_by_id_with_retries() {
    ...
    for ((i=0;  i<${GET_CALL_RETRIES}; i++)); do
        get_scan_result_code_by_id
        if [[ "${GET_CALL_STATUS}" == 200 ]]; then
            status=$(curl -s "${CURL_AUTH}" --header "Content-Type: application/json" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_ANCHORE_URL}/images/by_id/${SYSDIG_IMAGE_ID}/check?tag=$FULLTAG&detail=${DETAIL}" | grep "status" | cut -d : -f 2 | awk -F\" '{ print $2 }')
            break
        fi
        echo -n "." && sleep 1
    done
    ...
    printf "Scan Report - \n"
    curl -s "${CURL_AUTH}" --header "Content-Type: application/json" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_ANCHORE_URL}/images/by_id/${SYSDIG_IMAGE_ID}/check?tag=$FULLTAG&detail=${DETAIL}"
...
print_scan_result_summary_message() {
    if [[ ! "${V_flag-""}"  && ! "${R_flag-""}" ]]; then
        if [[ ! "${status}" = "pass" ]]; then
            echo "Result Details: "
            curl -s "${CURL_AUTH}" --header "Content-Type: application/json" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" "${SYSDIG_ANCHORE_URL}/images/by_id/${SYSDIG_IMAGE_ID}/check?tag=$FULLTAG&detail=true"
        fi
...
get_scan_result_pdf_by_digest() {
    ...
    curl -s "${CURL_AUTH}" --header "Content-Type: application/json" -H "Authorization: Bearer ${SYSDIG_API_TOKEN}" -o "${PDF_DIRECTORY}/${date_format}-${FULLTAG##*/}-scan-result.pdf" "${SYSDIG_SCANNING_URL}/images/${SYSDIG_IMAGE_DIGEST}/report?tag=$FULLTAG"
}
